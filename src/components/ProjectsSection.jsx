...
// Start with ARC only (add more later)
const PROJECTS = [
    {
        id: "arc",
        title: "ARC",
        type: "Website", // "Website" | "App"
        to: "/projects/arc",
    },
]

function ProjectCard({ title, to }) {
    return (
        <Link
            to={to}
            className={cn(
        ...
        >
            {/* Placeholder background (swap later with image) */}
            <div className="absolute inset-0 bg-gradient-to-b from-white/10 to-transparent" />

            {/* Hover darken overlay */}
            <div className="absolute inset-0 bg-black/10 transition-colors duration-300 group-hover:bg-black/60" />
            ...
            <div className="absolute inset-0 flex items-center justify-center">
                <div className="translate-y-2 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                    <div className="rounded-full border border-white/15 bg-black/40 px-5 py-2 text-sm tracking-wide text-white">
                        {title}
                    </div>
                </div>
            </div>

            {/* Optional “always visible” corner label (remove if you want) */}
        ...
        <section id="projects" className="relative z-10 mx-auto max-w-6xl px-6 py-12">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/40 p-8 backdrop-blur">
                ...
                <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {filtered.map((p) => (
                        <ProjectCard key={p.id} title={p.title} to={p.to} />
                    ))}
