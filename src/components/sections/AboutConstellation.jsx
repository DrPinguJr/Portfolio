...
function placeOnRing(items, cx, cy, r, startDeg = -90, sx = 1.0, sy = 1.0) {
    ...
    return items.map((s, i) => {
        ...
        return { ...s, x: cx + Math.cos(deg(a)) * r * sx, y: cy + Math.sin(deg(a)) * r * sy }
    })
}
...
export default function AboutConstellation() {
    ...
    const stars = useMemo(() => {
        ...
        return [{ ...north, x: ORBIT.north.x, y: ORBIT.north.y }, ...hardPlaced, ...softPlaced]
    }, [ORBIT.cx, ORBIT.cy, ORBIT.hardR, ORBIT.softR, ORBIT.sx, ORBIT.sy, ORBIT.north.x, ORBIT.north.y])
    ...
    const links = useMemo(() => {
        ...
        for (const [a, b] of extra) if (byId.has(a) && byId.has(b)) out.push([a, b])
    ...
    return (
        <section className="relative min-h-[calc(100vh-80px)] w-full overflow-hidden">
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 h-28 z-50 bg-gradient-to-b from-transparent to-black" />
